...
export type SecretOutletContext = {
  namespace: string
  name: string
  secret: SecretDetail
  invalidate: () => void
...
export default function SecretDetailLayout() {
  ...
  const query = useQuery({
    queryKey: [K8S_SECRETS_KEY, namespace, name],
    queryFn: () => getSecret(namespace, name),
    enabled: Boolean(namespace && name),
    refetchInterval: 8_000,
  })
  const invalidate = () => {
    void queryClient.invalidateQueries({ queryKey: [K8S_SECRETS_KEY] })
  }
  const base = `/kubernetes/secrets/${encodeURIComponent(namespace)}/${encodeURIComponent(name)}`
        ...
        {query.data?.data ? (
          <Outlet
            context={{
              namespace,
              name,
              secret: query.data.data,
              invalidate,
            } satisfies SecretOutletContext}
          />
        ) : null}
      </div>
    </ContentLoader>
