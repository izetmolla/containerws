...
import { getJob, K8S_JOBS_KEY, type JobDetail } from "../../_shared/api";
...
export default function JobDetailLayout() {
  ...
  const query = useQuery({
    queryKey: [K8S_JOBS_KEY, namespace, name],
    queryFn: () => getJob(namespace, name),
  ...
  const base = `/kubernetes/jobs/${encodeURIComponent(namespace)}/${encodeURIComponent(name)}`;
  return (
    <ContentLoader
      title={name}
      breadcrumb={[
