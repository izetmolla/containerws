...
export default function CreateDaemonSetPage() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (res) => {
      toast.success(res.message || "DaemonSet created")
      const ns = namespace.trim() || "default"
      navigate(
        `/kubernetes/daemonsets/${encodeURIComponent(ns)}/${encodeURIComponent(name.trim())}`,
      )
    },
    ...
    >
      <div className="w-full space-y-6">
        ...
        <p className="text-sm text-muted-foreground">
          Creates a DaemonSet that runs one pod per node. Selector labels are
          applied to both the DaemonSet selector and pod template.
        </p>
        ...
        <div className="flex flex-wrap justify-end gap-2">
          <Button variant="outline" asChild>
            <Link to="/kubernetes/daemonsets">Cancel</Link>
          </Button>
          <Button
            disabled={!canSubmit}
