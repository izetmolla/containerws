  ...
  return (
    ...
    >      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-zinc-800 px-3 py-2">
      ...
      {status === "error" && failureReason ? (
        <div className="border-b border-red-900/60 bg-red-950/50 px-3 py-2">
          <p className="text-[11px] font-medium uppercase tracking-wide text-red-300/90">
            Failure reason
          </p>
          <pre className="mt-1 whitespace-pre-wrap break-all font-mono text-[12px] leading-5 text-red-100">
            {failureReason}
          </pre>
        </div>
      ) : null}
      ...
      <div
        ref={scrollerRef}
        className={cn(
          "h-72 overflow-y-auto bg-[radial-gradient(ellipse_at_top,_#18181b_0%,_#09090b_55%)] px-3 py-3 font-mono text-[12px] leading-5",
          bodyClassName
        )}
        aria-live="polite"
      ...
      >
        {lines.length === 0 ? (
          ...
          <ul className="space-y-0.5">
            {lines.map((line) => (
              <li key={line.id} className={cn("whitespace-pre-wrap break-all", lineClass(line.stream))}>
